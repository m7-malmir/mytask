var tblLoanBankStatus = null;

$(function()
{
    tblLoanBankStatus = (function()
    {
        ...
        function addRow(rowInfo, rowNumber)
        {
			...
			/*var imgPrintAgreement = $("<img/>", {src: "Images/print.png", title: "حذف"}).addClass("printagreement").css({cursor: "pointer"});
            tempRow.find("td:eq(" + index++ + ")").append(imgPrintAgreement);
            tempRow.attr({state: "printed"});*/
			...
			tempRow.find("td:eq(" + index++ + ")").empty().text(rowInfo.PayStatus);
			
            if(rowPrimaryKeyName in rowInfo)
            {
                var reprint= $("<button/>", { title: "حذف"}).addClass("").css({cursor: "pointer"});
                tempRow.find("td:eq(" + index++ + ")").append(reprint);
        ...
        function load()
        {
            var params = {Where: "PayStatus = 1 and CancleStatus = 0"}; // change the params sent to FormManager with needed info
			showLoading();
