...
$("#CompanyId").getControl().select2({
 ...
 $("#submit0000000002").click(function(event){
  ...
  $("#submit0000000002").click(function (event) {
    alert('fuck uuuuuuuuuuuu');
    var rowCount = $("#Req_Grid").getNumberRows();   
  for (var i = 1; i <= rowCount; i++) {
   if($("#Req_Grid-body textarea[id='form[Req_Grid]["+i+"][Goods_Code]']").val().length === 0 || 
   $("#Req_Grid-body textarea[id='form[Req_Grid]["+i+"][Required_Items]']").val().length === 0 || $("#Req_Grid-body textarea[id='form[Req_Grid]["+i+"][Technical_Specifications]']").val().length === 0 || $("#Req_Grid-body textarea[id='form[Req_Grid]["+i+"][Number]']").val().length === 0){
      alert('تمامی موارد گرید تکمیل گردد!!!');
      event.preventDefault();
   }
  }
  });
  ...
  */
    /*this for check grid wont empty*/ 
  //#endregion
