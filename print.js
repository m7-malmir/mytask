    ...
    tblMain = (function()
    {
		...
		function bindEvents()
        {
			element.on("click", ".edit", function() {
				...
				var row = $(this).closest('tr'); // پیدا کردن ردیف
                var foodTitle = row.find('td:eq(3)').text().trim(); // دریافت نام غذا
                var rowIndex = '18'; // دریافت شماره ردیف

                // انتقال شماره ردیف به عنصر select
                var $cmbFoodTitle = $("#cmbFoodTitle");
                if ($cmbFoodTitle.find("option[value='" + rowIndex + "']").length) {
                    $cmbFoodTitle.val(rowIndex); // اگر گزینه وجود دارد، انتخاب کنید
                } else {
                    // اگر گزینه وجود ندارد، می‌توانید گزینه جدیدی اضافه کنید
                    $cmbFoodTitle.append(new Option(foodTitle, rowIndex)); // اضافه کردن گزینه جدید با شماره ردیف
                    $cmbFoodTitle.val(rowIndex); // انتخاب گزینه جدید
                }
